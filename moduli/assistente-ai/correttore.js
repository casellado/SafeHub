      ...
      <div x-show="chunkRecuperati.length > 0"
        ...
        <div x-show="tracciaAperta" class="divide-y divide-slate-100">
          <template x-for="c in chunkRecuperati" :key="c.id">
            <div x-data="{ aperto: false }" class="px-4 py-3">

              <!-- Sempre visibile: citazione + sintesi + fonte + toggle -->
              <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                  <p class="text-xs font-semibold font-mono text-slate-700 break-words"
                     x-text="c.riferimento"></p>
                  <p class="text-xs text-slate-600 font-medium mt-0.5" x-text="c.titolo"></p>
                ...
                <button @click="aperto = !aperto"
                        ...
                        x-text="aperto ? '▲ chiudi' : '▼ testo e nota'"></button>
              </div>

              <!-- A scomparsa: testo normativo + Nota CSE -->
              <div x-show="aperto" class="mt-3 space-y-2">

                <!-- Testo della norma -->
                <div class="bg-slate-50 border-l-2 border-slate-300 pl-3 py-2 rounded-r-md">
                  <p class="text-xs text-slate-700 whitespace-pre-wrap leading-relaxed"
                     x-text="c.testo"></p>
                </div>

                <!-- Nota CSE (solo se presente) -->
                <div x-show="c.note_cse"
                     class="bg-violet-50 border-l-2 border-violet-300 pl-3 py-2 rounded-r-md">
                  <p class="text-xs font-semibold text-violet-600 mb-1">&#x1F4A1; Nota CSE</p>
