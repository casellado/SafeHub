...
function ListaLavoratori() {
  return {
    ...
    async onExtraFile(event) {
      const file = event.target.files?.[0];
      if (!file) return;
      this.formExtra = { ...this.formExtra, filename: file.name, base64: await _leggiFileBase64Lav(file) };
    },
    ...
    salvaExtra() {
      ...
      // Soft-delete del record precedente in caso di modifica (stessa logica imprese)
    ...
    <div class="drawer-body px-5 py-4 space-y-3">
      ...
      <details class="border border-slate-200 rounded-xl overflow-hidden">
        ...
        <div class="p-4 space-y-3">
          ...
          <!-- Pulsante apri form + form inline add/modifica -->
          <div class="flex items-center justify-between mb-2">
            <span class="text-xs text-slate-400">
              Documenti aggiuntivi non previsti dallo schema (titolo libero, scadenza opzionale).
            </span>
            ...
            <div class="grid gap-2 sm:grid-cols-2">
              <div>
                ...
                <input type="text" x-model="formExtra.titolo"
                       placeholder="es. Dichiarazione CQOP, DUVC, verbale sopralluogo…"
                       class="w-full border border-slate-300 rounded px-2 py-1.5 text-xs
                              focus:outline-none focus:ring-2 focus:ring-blue-500">
              </div>
              <div>
