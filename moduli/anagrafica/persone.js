...
const _leggiFileBase64Persone = (file) =>
...
function ListaPersoneTerzi() {
  return {
    ..._factory('persone_terzi', 'Enti Terzi', '🏛'),

    // ── Documenti extra (raccoglitore libero — solo persone_terzi) ─────────
    mostraFormExtra:   false,
    idExtraInModifica: null,
    formExtra: { titolo: '', scadenza: '', filename: null, base64: null },

    // Override: inizializza documenti_extra sui record pre-esistenti
    modificaPersona(id) {
      const p = this.persone.find(x => x.id === id);
      if (!p) return;
      this.formDati = JSON.parse(JSON.stringify(p));
    ...
    get extraAttivi() {
      return (this.formDati.documenti_extra ?? []).filter(e => !e._cestino);
    },
    ...
    apriModificaExtra(id) {
      const ex = (this.formDati.documenti_extra ?? []).find(e => e.id === id && !e._cestino);
      if (!ex) return;
    ...
    async onExtraFile(event) {
      ...
      if (!file) return;
      this.formExtra = { ...this.formExtra, filename: file.name, base64: await _leggiFileBase64Persone(file) };
    },

    salvaExtra() {
      const titolo = (this.formExtra.titolo ?? '').trim();
      if (!titolo) return;
      if (!this.formDati.documenti_extra) this.formDati.documenti_extra = [];

      // Soft-delete del record precedente in caso di modifica
      if (this.idExtraInModifica) {
        const idx = this.formDati.documenti_extra.findIndex(e => e.id === this.idExtraInModifica);
        if (idx >= 0) {
          this.formDati.documenti_extra[idx] = {
            ...this.formDati.documenti_extra[idx],
            _cestino: true,
      ...
      this.formDati.documenti_extra.push({
    ...
    cestinaExtra(id) {
      const idx = (this.formDati.documenti_extra ?? []).findIndex(e => e.id === id && !e._cestino);
      if (idx < 0) return;
      this.formDati.documenti_extra[idx] = {
        ...this.formDati.documenti_extra[idx],
        _cestino: true,
...
<div x-data="ListaPersoneTerzi()" x-init="init()" x-effect="aggiornaSeCantiereRicambia()" class="max-w-4xl">
  ...
  <div x-show="drawerAperto" x-cloak @input="modDati=true" @keydown.escape.window="chiudiDrawer(false)"
    ...
    <div class="drawer-body px-5 py-4 space-y-3">
      ...
      <!-- 3. Altri documenti (raccoglitore libero) -->
      <details class="border border-slate-200 rounded-xl overflow-hidden">
        <summary class="px-4 py-3 bg-slate-50 cursor-pointer text-sm font-medium text-slate-700 hover:bg-slate-100 list-none flex items-center justify-between">
          <span>
            Altri documenti
        ...
        <div class="p-4 space-y-3">
          ...
          <div class="flex items-center justify-between mb-2">
            <span class="text-xs text-slate-400">Documenti aggiuntivi (titolo libero, scadenza opzionale).</span>
            ...
            <div class="grid gap-2 sm:grid-cols-2">
              <div>
                ...
                <input type="text" x-model="formExtra.titolo"
                       placeholder="es. Verbale sopralluogo, Autorizzazione…"
