...
function ListaImprese() {
  return {
    ...
    // ── Documenti extra (liberi, oltre i tipi fissi) ──────────────────────────
    ...
    async onExtraFile(event) {
      const file = event.target.files?.[0];
      if (!file) return;
      this.formExtra = { ...this.formExtra, filename: file.name, base64: await _leggiFileBase64Imp(file) };
    },
    ...
    salvaExtra() {
      ...
      // Soft-delete del record precedente (modifica) — stessa logica dei tipi fissi
                ...
                <div class="grid gap-2 sm:grid-cols-2">
                  <div>
                    ...
                    <input type="text" x-model="formExtra.titolo"
                           placeholder="es. Comunicazione INAIL, Contratto nolo…"
                    ...
                    <div class="min-w-0 flex-1">
                      <div class="flex items-center gap-2 flex-wrap">
                        ...
                        <span x-show="ex.scadenza" class="font-normal"
                              :class="ex.scadenza && UTILS.giorniAllaScadenza(ex.scadenza) < 0
